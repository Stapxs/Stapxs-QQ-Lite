...
// 全局变量
window.version = 'v2.0'             // 版本号
window.is_auto_dark = true          // 自动暗黑模式标志
window.debug_mode = true            // 开发者模式
...
// 样式 LOG
function showLog(head, info) {
    head = head.toUpperCase()
    // 初始样式
    let bg = "99b3db"
    let fg = "ffffff"
    switch(head) {
        case "ERR": bg = "ff5370"; break;
        case "UI": bg = "b573f7"; break;
        case "GET": bg = "4a93c3"; break;
        case "PUT": bg = "7abb7e"; break;
    }
    // 输出
    const level = window.optCookie == undefined ? undefined : window.optCookie["opt_log_level"]
    if(((level == undefined || level == "err") && head == "ERR") || level == "all" || head == "SS" || window.debug_mode) {
